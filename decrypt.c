...
void initialTransformation_d(uint8_t * st,uint8_t * WK1) 
{
    uint8_t i;
 ...
 void Round_d(uint8_t * st,uint8_t * sk,uint8_t j) 
{
    ...
    x[7]=((st[0])^sk[Index])^st[7];  
  
    x[0]=st[1];
    Index++;
    
    x[1]=(~(st[2]^sk[Index]))^st[1];

    x[2]=st[3];
    Index++;
    
    x[3]=((st[4])^sk[Index])^st[3];

    x[4]=st[5];
    Index++;
    
    x[5]=(~(st[6])^sk[Index])^st[5];
 ...
 void FinalTransformation_d(uint8_t * st,uint8_t * WK1) 
{
    uint8_t i;
...
void Decrypt(uint8_t *block, uint8_t *rK)
{
         uint8_t i;
   uint8_t sk1[80];     
 //==============================
       initialTransformation_d(block,rK);
     ...
     FinalTransformation_d(block,rK);
//==============================
